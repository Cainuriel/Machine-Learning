...
""" he legal moves genator accepts the board state and player as input,
and provides the set of all possible legal moves as output.

Basically, for a given board state and player, 
it tells us where all the player can place his mark legally 
(the player can only place the mark where there is no opponent's mark) """
...
We start a dummy game, and do the toss. 
We then pass the current board state and toss winner to the Legal Moves Generator,

It returns the dictionary of legal moves that it returns

The dictionary is of the form 
"possible next legal move coordinate":"flattened resulting board state"""
...
below, we run a single iteration of training, or in other words, 
a single game of tic-tac-toe between the program and opponent. """
